...
def crop_spmel(spmel, len_crop):
    if spmel_tmp.shape[0] < len_crop:
        len_pad = len_crop - spmel_tmp.shape[0]
        spmel = np.pad(spmel_tmp, ((0,len_pad),(0,0)), 'constant')
    elif spmel_tmp.shape[0] > len_crop:
        left = np.random.randint(spmel_tmp.shape[0]-len_crop)
        spmel = spmel_tmp[left:left+len_crop, :]
    else:
        spmel = spmel_tmp
    return spmel
        ...
        if file_name.startswith(gender):
            if row[0] != 2: # we look at the 
                if t_src in file_name:
                    ...
                    spmel = crop_spmel(spmel_tmp, len_crop)
