        ...
        def batch_iterate():
            ...
            for i in range(current_iter+1, (current_iter+1 + cycle_size)):
                ...
                if i % self.config.log_step == 0 or i == (current_iter + cycle_size):
                    ...
                    if mode == 'train':
                        log = "Elapsed [{}], Mode {}, Iter [{}/{}]".format(et, mode, i, self.config.num_iters)
                ...
                if mode == 'train':
                    ...
                    if i % self.config.spec_freq == 0:
                        ...
                        for j in range(0, columns*rows):
                            ...
                            plt.title(name)
                            plt.colorbar()
                        plt.savefig(self.config.data_dir +'/' +self.config.file_name +'/image_comparison/' +str(i) +'iterations')
                        plt.close()
        
                    ckpt_path = self.config.data_dir +'/' +self.config.file_name +'/ckpts/' +'ckpt_' +str(i) +'.pth.tar'
